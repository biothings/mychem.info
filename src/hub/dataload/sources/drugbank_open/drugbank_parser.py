            ...
            if isinstance(value, collections.OrderedDict):
                for x, y in iter(value.items()):
                    for ele in y:
                        for name in ele:
                            if name == "text":
                                synonym_list.append(ele[name].strip())
                                print(d1)
                        ...
                        try:
                            ...
                            try:
                                for i, val in enumerate([v for v in vals]):
                                    vals[i] = float(val)
                                logging.info(
                                    "Document ID '%s' has a range " % restr_dict["_id"]
            ...
            def restr_reaction_dict(dictionary):
                ...
                if dictionary.get("enzymes"):
                    if isinstance(dictionary["enzymes"]["enzyme"], list):
                        ...
                        _dict["enzymes"] = dict(dictionary["enzymes"]["enzyme"])
                return _dict
...
if __name__ == "__main__":
    xml_file_path = "drugbank.xml"
    for doc in load_data(xml_file_path):
        print(doc)  # or you can save it, or do any processing you want
        pass
